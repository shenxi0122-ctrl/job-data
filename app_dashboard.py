...
st.subheader("一、招聘趋势")
# 修正后安全语法
monthly_df = filtered.groupby("年月", dropna=False)[["jobid", "平均薪资"]].agg({"jobid":"count","平均薪资":"mean"}).reset_index()
monthly_df.rename(columns={"jobid":"招聘数量"},inplace=True)
monthly_df = monthly_df.sort_values("年月")

trend = go.Figure()
trend.add_scatter(x=monthly_df["年月"], y=monthly_df["招聘数量"], name="招聘数量", mode="lines+markers")
trend.add_scatter(
    x=monthly_df["年月"],
    y=monthly_df["平均薪资"],
    name="平均薪资",
